...
{
    public partial class RelatorioKatTecnicoVisita : ReportBasePage
    {
        ...
        protected override void ExibirRelatorio()
        {
            ...
            DataSet dsRptKatPorTecnicoVisita = new KatTecnicoData().ObterRelatorioVisita(dataInicial, dataFinal, codigoTecnicos, codigoClientes);

            var reportDS = new ReportDataSource();
            reportDS.Name = "dsRptKatPorTecnicoVisita";
            reportDS.Value = dsRptKatPorTecnicoVisita.Tables[0];

            if (dsRptKatPorTecnicoVisita.Tables[0].Rows.Count == 0)
            {
            ...
            ReportViewer1.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"\Reports\KatTecnicoVisita\RptKatPorTecnicoVisita.rdlc";
