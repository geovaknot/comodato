...
namespace _3M.Comodato.API.Controllers
{
    ...
    public class PedidosAPIController : BaseAPIController
    {
        ...
        [HttpPost]
        [Route("ObterLista")]
        public IHttpActionResult ObterLista(PedidosEntity pedidosEntity)
        {
            try
            {
                JObject JO = new JObject();
                JO.Add("STATUS", JsonConvert.SerializeObject(new PedidosData().ObterLista(pedidosEntity), Formatting.None));
                return Ok(JO);
            }
            catch (Exception ex)
            {
